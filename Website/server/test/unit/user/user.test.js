import createUser from "../user/userFactory.js";
import userInterface from "./userInterface.js";
import {expect} from "chai";

describe("User Entity", ()=>{
	let user;
	let userWrapper;
	
	beforeEach(()=>{
		// Used the user factory to instantiate a new user.
		user = createUser("1", "test123", "test123", "test123@gmail.com");
		// Dependency injection. I've made this test file dependent on the userWrapper interface for interacting with the user object.
		userWrapper = new userInterface(user);
	});
	
	it("should have an id, username, password, and email property", ()=>{
		expect(user).to.have.property("id").that.equals("1");
		expect(user).to.have.property("username").that.equals("test123");
		expect(user).to.have.property("password").that.equals("test123");
		expect(user).to.have.property("email").that.equals("test123@gmail.com");
	});
	
	it("should have a username with at least three characters", ()=>{
		expect(userWrapper.getValidateUserName()).to.be.true;
	});
});