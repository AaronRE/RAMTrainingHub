...
describe("User Factory",()=>{
	...
	it("should successfully create a user", async()=>{
		res = await createUser("test", "test", "test12345", "test12345", "test123@publix.com");
	...
	it("it should handle when the first name isn't entered", async() =>{
		res = await createUser("", "test", "test12345", "test12345", "test123@publix.com");
		expect(res.boolean).to.be.false;
	});
	
	it("it should handle when the first name is too long", async() =>{
		res = await createUser("t".repeat(16), "test", "test12345", "test12345", "test123@publix.com");
		expect(res.boolean).to.be.false;
	});
	...
	it("it should handle when the last name is too long", async() =>{
		res = await createUser("test", "t".repeat(16), "test12345", "test12345", "test123@publix.com");
		expect(res.boolean).to.be.false;
	});
	
	it("it should handle when the username is too short", async() =>{
		res = await createUser("test", "test", "t".repeat(6), "test12345", "test123@publix.com");
		expect(res.boolean).to.be.false;
	});
	
	it("it should handle when the username is too long", async() =>{
		res = await createUser("test", "test", "t".repeat(30), "test12345", "test123@publix.com");
		expect(res.boolean).to.be.false;
	});
	
	it("it should handle when the password is too short", async() =>{
		res = await createUser("test", "test", "test12345", "t".repeat(7), "test123@publix.com");
		expect(res.boolean).to.be.false;
	});
	
	it("it should handle when the password is too long", async() =>{
		res = await createUser("test", "test", "test12345", "t".repeat(20), "test123@publix.com");
		expect(res.boolean).to.be.false;
	});
	
	it("it should handle when the email doesn't end with @publix.com", async() =>{
